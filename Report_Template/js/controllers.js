var myApp = angular.module('myApp', ['ngAnimate', 'smart-table', 'ngRoute', 'ngSanitize']);
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            scope.$watch('chartType', function(value) {
                ...
                chartObj = chart[getChartName(scope.chartType)](scope.chartData,{ animation: true, animationEasing: "easeInSine" , animationSteps :"39" });                
...
myApp.filter('statusToI18N', function() {
    return function(status) {
        var text = angular.lowercase(status) == "not_run" ? "notRun" : angular.lowercase(status);
...
myApp.controller('PageController', ['$scope', '$window', function($scope, $window) {
    ...
    $scope.getMessageColor = function(data) {
        var color = data.substring(data.lastIndexOf(' ') + 1);
        ...
        if (color == reportLang.Pink) {
            ...
            data = "<span class=\"black\">" + data + "</span>";
    ...
    $scope.filterNav = function(type) {
        if (type != undefined && type.length > 0) {
            angular.element(document.querySelectorAll('[data-status]')).addClass('hi‌​de');
            angular.element(document.querySelectorAll('[data-status="' + type + '"]')).removeClass('hi‌​de');
            $scope.filterNavActive = type;
        } else {
            angular.element(document.querySelectorAll('[data-status]')).removeClass('hi‌​de');
            $scope.filterNavActive = '';
        }
    }
...
myApp.controller('HomeController', ['$scope', '$http', function($scope, $http) {
    ...
    $scope.chartResult = [$scope.reportData.result.pass, $scope.reportData.result.fail, $scope.reportData.result.skip, $scope.reportData.result.notRun];
    $scope.itemsByPage = 5;
}]);
myApp.controller('SuiteController', ['$scope', '$http', '$routeParams', function($scope, $http, $routeParams) {
    $scope.suiteData = results.childObjects[$routeParams.suiteIndex];
    $scope.caseList = $scope.suiteData.childObjects;
    $scope.chartResult = [$scope.suiteData.result.pass, $scope.suiteData.result.fail, $scope.suiteData.result.skip, $scope.suiteData.result.notRun];
    $scope.itemsByPage = 5;
}]);

myApp.controller('CaseController', ['$scope', '$http', '$routeParams', function($scope, $http, $routeParams) {
    $scope.caseData = results.childObjects[$routeParams.suiteIndex].childObjects[$routeParams.caseIndex];
    $scope.steps = $scope.caseData.childObjects;

    $scope.checkIterartion = function(currentType, previousType) {
        if (typeof(previousType) != "undefined" && (currentType.indexOf('Iteration-') > -1)) {
            if ((previousType.indexOf('Iteration-') > -1) && (currentType.indexOf('Iteration-') > -1) && previousType == currentType) {
                return false;
            } else {
                return true;
            }
    ...
    $scope.checkCallMethod = function(currentType, previousType) {
        if ((typeof(previousType) != "undefined") && (currentType.indexOf('CallMethod') > -1)) {
    ...
    $scope.checkIterartionClass = function(currentType) {
        if ((currentType.indexOf('CallMethod') > -1) || (currentType.indexOf('Iteration-') > -1)) {
            return 'iteration-row';
        }
    }

    $scope.checkIterartionIsBlank = function(index) {
        if ((($scope.steps[index].type.indexOf('Iteration-') > -1) || ($scope.steps[index].type.indexOf('CallMethod') > -1)) && ($scope.steps[index].operation.name.length == 0) && ($scope.steps[index].executionTime == 0)) {
            return true;
        }
        return false;
    }
...
myApp.controller('SummaryChartController', ['$scope', '$http', function($scope, $http) {
    ...
    function summaryBarChart() {
        ...
        $scope.chartData = {
            ...
            datasets: [            
                    {   
                        data : [$scope.$parent.chartResult[0], $scope.$parent.chartResult[1], $scope.$parent.chartResult[2], $scope.$parent.chartResult[3]]                                           
                    }, 
                ],
            };                  
         }
...
myApp.controller('TrendChartController', ['$scope', '$http', function($scope, $http) {
...
myApp.controller('EffectivenessChartController', ['$scope', '$http', function($scope, $http) {
    ...
    $scope.chartData = {
        labels: ["Suite 1", "Suite 2", "Suite 3", "Suite 4", "Suite 5", "Suite 6", "Suite 7"],
        datasets: [{
                label: "Steps",
